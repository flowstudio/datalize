...
class Field {
	...
	processMessage(message, ctx, options) {
		if (options.translator) {
			message = (typeof options.translator === 'function' ? options.translator : ctx[options.translator]).apply(this, Array.isArray(message) ? message : [message]);
		}
	...
	async validateArray(value, result, ctx, options) {
		...
		if (this.isRequired && !value.length) {
			return {value, errors: this.addError([], new Error(filters.required.message))};
		}
	...
	async validateField(value, result, ctx, options) {
		...
		for (const fn of this.stack) {
			try {
				...
				if (isRequired || !filters.isEmpty(value)) {
					err.message = this.processMessage(err.message, ctx, options);

					this.addError(errors, err);
				}
		...
		if (errors.length && this.message) {
			errors = [this.processMessage(this.message, ctx, options)];
		}
	...
	async validate(source, result, ctx, options) {
		const errors = [];
		let value = get(this.isFile ? source.__files : source, this.name);

		if (this.isArray && !value) {
			// try to load data from name[] key
			value = get(this.isFile ? source.__files : source, this.name + '[]');
		}
...
const extend = [
	...
	'int', 'float', 'id', 'select', 'min', 'max', 'isEmail',
