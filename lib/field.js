...
class Field {
	...
	async validateField(value, result, ctx, options) {
		...
		for (const {fn, message} of this.stack) {
			try {
				const ret = await fn(value, message, result, ctx, this);
	...
	required(message) {
		...
		return this.add(filters.required, message);
	}
...
extend.forEach(name => {
	Field.prototype[name] = function(...args) {
		var message;
		if (args.length && args[args.length-1].message) {
			message = args[args.length-1].message;
		}
		if (!args.length) {
			return this.add(filters[name]);
		}
		if (args.length == 1 && message) {
			return this.add(filters[name], message);
		}
		if (message) {
			args.pop();
		}
		return this.add(filters[name].bind(null, ...args), message);
	};
});
